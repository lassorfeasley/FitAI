...
const ActiveFocusedNavBar = ({ timer, isPaused, onPauseToggle, onEnd }) => {
  return (
    ...
    >
      <div
        data-layer="MaxWidthWrapper"
        className="Maxwidthwrapper w-80 max-w-80 flex justify-between items-start"
      >
        <div
          data-layer="Timer"
          className="Timer flex justify-start items-center gap-1"
        >
          <div
            data-svg-wrapper
            data-layer="RecordingIcon"
          ...
          <div
            data-layer="TimePassed"
            className="Timepassed justify-center text-white text-xl font-normal font-['Space_Grotesk'] leading-loose"
          >
            {timer}
          </div>
        </div>
        <div
          data-layer="NavIconsWrapper"
          className="Naviconswrapper flex justify-start items-center"
        >
          <div
            data-layer="NavIcons"
            data-selected={!isPaused}
            className={`Navicons w-16 inline-flex flex-col justify-start items-center gap-1 cursor-pointer${
              isPaused ? " NaviconsSelected3 w-14" : ""
            }`}
          ...
          >
            {isPaused ? (
              <>
                ...
                <div
                  data-layer="Resume"
                  className="Resume text-center justify-start text-white text-xs font-bold font-['Space_Grotesk'] leading-3"
                >
                  Resume
                </div>
              </>
...
const ActiveWorkout = ({
  ...
  const [newUnscheduledExerciseName, setNewUnscheduledExerciseName] =
    useState("");
  const [newUnscheduledExerciseSets, setNewUnscheduledExerciseSets] =
    useState(3);
  const [newUnscheduledExerciseReps, setNewUnscheduledExerciseReps] =
    useState(10);
  const [newUnscheduledExerciseWeight, setNewUnscheduledExerciseWeight] =
    useState(0);
  const [newUnscheduledExerciseUnit, setNewUnscheduledExerciseUnit] =
    useState("kg");
    ...
    if (selectedProgram) {
      ...
      supabase
        ...
        .then(async ({ data: progExs, error }) => {
          ...
          if (exercisesError) {
            console.error(
              "Error fetching exercises for program:",
              exercisesError
            );
            setExercises([]);
            setLoading(false);
  ...
  useEffect(() => {
    setNewUnscheduledSetConfigs((prev) => {
      const arr = Array.from(
        { length: newUnscheduledExerciseSets },
        (_, i) =>
          prev[i] || {
    ...
    newUnscheduledExerciseSets,
    newUnscheduledExerciseReps,
    newUnscheduledExerciseWeight,
    newUnscheduledExerciseUnit,
  ]);
  ...
  return (
    <>
      <AppHeader
        ...
        subhead={true}
        subheadText={
          selectedProgram?.program_name || "Tracking unscheduled exercises"
        }
        search={false}
        showActionBar={true}
          ...
          {exercises.map((ex) => (
            <ActiveExerciseCard
              ...
              onSetComplete={(setDataArg) =>
                handleSetComplete(ex.exercise_id, setDataArg)
              }
